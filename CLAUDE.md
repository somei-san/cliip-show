...
- 意図したUI変更（HUD外観の変更など）: VRTのベースラインを更新する
  ...
  ```
- VRT のケースを追加しただけのとき: `--update` ではなく `--update-missing`（使い分けは `docs/development.md`）
- VRT が撮るのは HUD の contentView、設定ウィンドウの各ペイン、設定タブの行スタック全体（`run_settings_case`。ペインはタブバーが contentView の外なので写らず、行スタック全体は `NSScrollView` の documentView を可視域でクリップせず撮る）。メニューバーと About パネルは対象外（システム所有の表面で view を PNG 化できない。About は渡す中身を UT で固定している）
