...
ログイン時の自動起動は設定ウィンドウの「ログイン時に自動起動」からいつでも切り替えられます（[表示設定](#表示設定)を参照）。

## 表示設定

メニューバーの「設定…」から設定ウィンドウを開いて変更します。設定を変更する手段はこれだけです。ウィンドウは「設定」「寄付」の2つのタブに分かれていて、設定項目は「設定」タブにあります。
